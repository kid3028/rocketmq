...
/**
 * namesrv通过将broker注册来的信息构造成自己的数据结构
 * 每个cluster有哪些broker set
 * 每个broker set包括哪些broker，brokerId和broker的ip:port
 * 每个broker的存活情况，根据每次broker上报上来的信息，清除可能下线的broker
 * 每个topic的消息队列信息，几个读队列，几个写队列
 *
 * namesrv汇总所有broker的这些信息，然后供consumer和producer拉取
 */
public class RouteInfoManager {
    ...
    // 每个broker set中的broker信息(set中有哪些broker，每个broker的brokerId、brokerAddr)
    ...
    public RegisterBrokerResult registerBroker(
        ...
        try {
            try {
                ...
                // 如果当前集群还没有broker注册信息，创建map存储该集群broker列表
                if (null == brokerNames) {
                    brokerNames = new HashSet<String>();
                    this.clusterAddrTable.put(clusterName, brokerNames);
                }
                // 将注册的broker加入集群broker列表
                brokerNames.add(brokerName);
                // 标记是否为第一次注册
                boolean registerFirst = false;

                // 如果从broker元数据列表中没有找到该broker的信息，说明broker是第一次注册
                ...
                // 设置broker地址。向broker的地址列表中添加brokerId，broker地址的关系映射
                String oldAddr = brokerData.getBrokerAddrs().put(brokerId, brokerAddr);
                registerFirst = registerFirst || (null == oldAddr);

                /**
                 * 只有master才会创建queueData，因为只有master才包含了读写队列的信息
                 * Slave没有自己独立的读写队列信息，Slave不会创建自己的queue信息，只是和master的读写队列信息一致
                 */
                if (null != topicConfigWrapper
                    && MixAll.MASTER_ID == brokerId) {
                    // 配置修改或者第一次注册，需要创建或者更新队列数据
                ...
                if (filterServerList != null) {
                    // 判断过滤器服务列表是否为空来确定在broker地址 --> 过滤服务里诶表 映射中移除或者添加映射
                ...
                // 不是master，也就是slave broker，需要拿到master broker的信息
    ...
    private void createAndUpdateQueueData(final String brokerName, final TopicConfig topicConfig) {
        ...
        // 获取到topicName相关信息 topicName --> 队列数据列表
    ...
    public void unregisterBroker(
        ...
        try {
            try {
                this.lock.writeLock().lockInterruptibly();
                // 从 " broker地址 --> 存活的broker信息" 映射中移除指定地址的映射
                ...
                BrokerData brokerData = this.brokerAddrTable.get(brokerName);
                if (null != brokerData) {
                    // 从broker元数据中 "brokerId --> brokerAddr"列表中移除指定的brokerId对应的broker地址
                ...
                if (removeBrokerName) {
                    ...
                    if (nameSet != null) {
                        ...
                        // 如果移除broker后集群broker列表为空，则需要移除此集群
                        if (nameSet.isEmpty()) {
    ...
    private void removeTopicByBrokerName(final String brokerName) {
        ...
        while (itMap.hasNext()) {
            ...
            // 如果移除后队列列表为空，则移除此"topic -> 队列数据信息" 映射信息
     ...
     *    2.2.将broker从brokerAddrTable移除
     *    2.3.将broker从集群中移除
     *    2.4.将broker从topicQueue中移除
     * @param remoteAddr
     * @param channel
     */
            ...
            try {
                try {
                    ...
                    while (itBrokerAddrTable.hasNext() && (null == brokerNameFound)) {
                        ...
                        // 如果移除之后BrokerData中的地址列表为空，则次brokerName需要移除
                        if (brokerData.getBrokerAddrs().isEmpty()) {
                            removeBrokerName = true;
                            // 从地址列表中移除
