...
public class MQClientAPIImpl {
    ...
    /**
     * 从BrokerController拉取某个消费组下的消费者列表
     * @param addr
     * @param consumerGroup
     * @param timeoutMillis
     * @return
    ...
    /**
     * consumer向broker发送回消费失败的消息
     *
     * 发送重试的消息的时候会有几个关键属性
     *    originTopic：因为重试的消息被broker拿到后会修改topic，投递到新的topic下，所以需要保留一个原始的topic
     *    delayLevel：该消息是否会被延时消费
     *    maxReconsumeTime：这个消息最多可以重试多少次
     * @param addr
     * @param msg
     * @param consumerGroup
     * @param delayLevel
    ...
    public void consumerSendMessageBack(
        ...
        // 因为重试的消息会被broker拿到后会修改topic，所以治理设置原始的topic
        requestHeader.setOriginTopic(msg.getTopic());
        // broker会根据offset查询原始的消息
        requestHeader.setOffset(msg.getCommitLogOffset());
        // 设置delayLevel，这个值决定了该消息是否会被延迟消息，延迟多久
        // 用户可以设置延迟等级，默认是0，不延迟(但是broker端会有逻辑，如果为0会增加3)
